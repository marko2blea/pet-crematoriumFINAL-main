<template>
  <div class="pt-14 py-20 min-h-screen container mx-auto px-4 animate-page-fade">
    ...
    <div
      v-if="pending"
      class="text-center p-10 bg-white rounded-xl shadow-lg animate-fade-in-up"
    >
      <h1 class="text-3xl font-bold text-dark-primary-blue flex items-center justify-center gap-2">
    ...
    <div
      ...
      class="text-center p-10 bg-red-50 rounded-xl shadow-lg border border-red-300 animate-fade-in-up"
    >
      <h1 class="text-3xl font-bold text-red-700">Error al Cargar el Usuario</h1>
      <p class="text-gray-600 mt-2">
        {{ error?.statusMessage || 'El usuario no pudo ser encontrado.' }}
      </p>
      <button
        @click="router.push('/admin/gestionar-usuario')"
        class="mt-6 px-5 py-2 bg-purple-dark text-white rounded-lg hover:bg-purple-deep transition shadow-lg transform hover:-translate-y-0.5 hover:shadow-xl"
      >
        Volver a Gestión
      </button>
    </div>
    ...
    <form
      ...
      class="max-w-3xl mx-auto bg-white rounded-xl shadow-2xl overflow-hidden border-t-8 border-purple-dark animate-scale-in"
    >
      <!-- Header -->
      <div class="p-6 bg-gray-50 border-b border-gray-200">
        ...
        <p class="text-sm font-mono text-purple-deep">
          {{ form.correo }} <span class="text-gray-500">(No editable)</span>
        </p>
      </div>
      ...
      <div
        ...
        class="m-6 p-4 rounded-lg border text-sm font-medium text-center animate-fade-in-up"
      >
        {{ saveMessage }}
      </div>
      ...
      <div class="p-6 md:p-8 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div class="animate-field-fade delay-1">
            <label
          ...
          <div class="animate-field-fade delay-2">
            <label
        ...
        <div class="animate-field-fade">
          ...
          >
            <option v-if="!roles || roles.length === 0" disabled value="">
              Cargando roles...
            </option>
            <option
              v-for="rol in roles"
      ...
      >
        <button
          type="button"
          @click="router.push('/admin/gestionar-usuario')"
          class="px-5 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-100 transition duration-150 transform hover:-translate-y-0.5"
        >
          Cancelar
        </button>
        <button
          type="submit"
          :disabled="isSaving"
          class="px-5 py-2 bg-purple-deep text-white rounded-lg hover:bg-purple-light transition duration-150 shadow-md disabled:opacity-50 disabled:cursor-not-allowed transform hover:-translate-y-0.5 hover:shadow-lg"
        >
...
// --- Cargar Datos ---
const { data: loadedData, pending, error } =
  await useAsyncData<DetalleUsuarioResponse>(
    'usuario-detalle',
    async () => {
      if (!usuarioId.value) {
        throw createError({
          statusCode: 400,
          statusMessage: 'Falta ID de usuario'
        });
      }
      return await $fetch<DetalleUsuarioResponse>(
        '/api/admin/usuario-detalle',
        { query: { id: usuarioId.value } }
      );
    },
    { watch: [usuarioId] }
  );
...
const guardarCambios = async () => {
  ...
  try {
    ...
    saveMessage.value =
      err.data?.statusMessage || 'Error al guardar el usuario.';
  }
};
</script>
...
/* Títulos de sección con ligeras animaciones */
.section-title {
  @apply text-xl font-semibold text-purple-deep border-b pb-2 flex items-center gap-2;
  animation: fade-in-up 0.35s ease-out;
}

/* Pequeñas animaciones en campos */
.animate-field-fade {
  animation: field-fade 0.35s ease-out;
}
.animate-field-fade.delay-1 {
  animation-delay: 0.05s;
}
.animate-field-fade.delay-2 {
